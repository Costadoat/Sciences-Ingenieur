...
\hline
Traitement numérique de la vitesse & $K_{Vit} = 1.4\cdot 10^{-3}\ V\cdot rad \cdot s^{-1}$ \\
\hline
...
\reponse{2}{}{$E=5$ et $\omega_n=\frac{2\cdot \pi\cdot 2,5}{0.04}=2\cdot \pi\cdot 2,5\cdot 25\approx 400rad\cdot s^{-1}$}

\reponse{0}{\begin{center}
  \resizebox{0.6\textwidth}{!}{\input{img/q6_DR.pgf}}
\end{center}
}{
D'après le diagramme de Bode, $G_{db}(\omega_n)=-20db$, donc $G(\omega_n)=0,1$ et $\varphi(\omega_n)=-135\degree$, donc, $s(t)=1\cdot \sin(400\cdot t-135)$. Il faut donc tracer un signal dont l'amplitude est de 1, la période est la même que l'entrée et il y a un retard de $\frac{135}{360}\approx 0.4$ période.
\begin{center}
...
Pour:
\begin{itemize}
 \item $K_p=0.1$: $t_{r5\%}=0.8s$ et $a=\frac{0.3}{0.35}=0.86m\cdot s^{-2}$,
 \item $K_p=0.2$: $t_{r5\%}=0.4s$ et $a=\frac{0.3}{0.35}=2m\cdot s^{-2}$,
 \item $K_p=0.3$: $t_{r5\%}=0.25s$ et $a=\frac{0.3}{0.35}=1m\cdot s^{-2}$, 
 \end{itemize}
 
Aucune valeur ne convient alors car $a$ est toujours plus grand que $0.8m\cdot s^{-2}$.}
